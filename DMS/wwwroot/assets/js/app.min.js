...
(function ($) {
  ...
  table_responsive_stack.each(function () {
    ...
    if ($('#justgage_five').length) {
        ...
        xmlHttp.send();
        var storage = JSON.parse(xmlHttp.responseText);


        var maxStorage = formatBytes(storage.Storage * 1048576.0);
        if (maxStorage == "NaN undefined")
            maxStorage = "Infinity";

      new JustGage({
          id: 'justgage_five',
          value: maxStorage == "Infinity" ? 0 : (storage.UsedStorage / storage.Storage) * 100,
      minTxt: " ",
      min: 0,
      max: 100,
      maxTxt: " ",
      symbol: '%',
          label: formatBytes(storage.UsedStorage * 1048576.0) + " out of " + maxStorage,
      gaugeWidthScale: 0.7,
      counter: true,
