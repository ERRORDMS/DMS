...
var RuleRangesEditor = (function () {
    function RuleRangesEditor(options) {
        ...
        _knockout_utils_1.safeComputed({ valueType: this.condition.valueType }, function () {
            _this._updateValue();
            _this.updateRangeValues(_this.isPercent());
        });
        this.condition.rangeSet.ranges.subscribe(function () { return _this._updateValue(); }, null, 'arrayChange');
    }
    RuleRangesEditor.prototype.updateRangeValues = function (isPercent) {
        var valueType = isPercent ? 'System.Double' : _format_rules_common_1.fieldTypes[this.dataType()];
        this.condition.rangeSet.ranges().forEach(function (rangeInfo) { return rangeInfo.value.setValue(RuleRangesEditor.patchValueToMatchSerializedType(rangeInfo.value.value(), valueType), valueType); });
    };
    ...
    RuleRangesEditor.prototype.createRangeNumberEditorViewModel = function (args) {
        ...
        var editorValue = ko.observable(isNegativeInfintySelected ? 0 : args.value());
        var allowInfinity = args.isRightValue && args.grid.totalCount() === args.rowIndex + 1 && !this.isPercent();
        var infinitValue = ko.observable(isNegativeInfintySelected);
        var editorType = this.dataType() === 'DateTime' ? 'dateBox' : 'numberBox';
        var editorOptions = {
    ...
    RuleRangesEditor.patchValueToMatchSerializedType = function (value, valueType) {
